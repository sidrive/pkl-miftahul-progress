...
- Event Handling, Binding, & Komposisi Komponen (dipecah jadi 4 sub-task):
  - [ ] <!-- id: M3.W1.T3.1 --> `@click`/`v-on` — **2 putaran, kasus berbeda**: putaran 1 ulangi pola minggu lalu (`@click.prevent`); putaran 2 pakai event BARU (misal `@input` di form) untuk kasus yang beda (bukan sekadar ganti nama event di kasus yang sama)
  - [ ] <!-- id: M3.W1.T3.2 --> `:class`/`v-bind` dinamis — **2 putaran, kasus berbeda**: putaran 1 pakai `:class="{ selesai: item.selesai }"` untuk styling kondisional (mengingatkan ke `classList.add`/`toggle` manual di vanilla JS dulu); putaran 2 ulangi dari nol dengan **1 kondisi/data lain** yang belum dipakai di putaran 1
  - [ ] <!-- id: M3.W1.T3.3 --> Komposisi komponen — **BARU, 2 putaran**: putaran 1 bikin 1 child component (beda dari `Tentang.vue` minggu lalu) yang **kirim data dari parent ke child lewat `props`** (`defineProps`) — beda dari `Tentang.vue` kemarin yang isinya statis, sekarang datanya harus mengalir dari `App.vue`; putaran 2 bikin child component KEDUA dengan `props` berbeda (bukan reuse komponen yang sama, biar terbukti paham polanya, bukan hafal 1 komponen)
