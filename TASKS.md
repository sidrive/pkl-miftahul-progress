...
- Commit history rapi & alur kerja yang benar (dipecah jadi 4 sub-task — **ini menegakkan lagi pelajaran dari insiden `T4.3`/`T4.4` Minggu 3**, di mana PR sempat ditandai ter-merge padahal belum pernah direview):
  ...
  - [ ] <!-- id: M1.W4.T5.3 --> **[Wajib — jangan ditandai `done` sebelum benar-benar terjadi]** Buat PR, dan **tunggu mentor benar-benar memberi review nyata di GitHub** sebelum merge — kalau mentor belum sempat review, task ini `in-progress`, BUKAN `done`
