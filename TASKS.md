...
>
> ⚠️ **Catatan penting dari evaluasi Minggu 1 Bulan 2:** kelihatan pola "yang penting task ke-checklist selesai" — kode sempat jalan sekali, tidak diulang/didalami, akibatnya beberapa fungsi yang **kamu buat sendiri** tidak bisa kamu jelaskan lagi saat ditanya, dan sempat coba-coba/lihat modul lagi untuk jawab. Mulai `reduce`/`find` (Rabu) di bawah, **setiap konsep WAJIB dipraktikkan ulang minimal 1x lagi dengan kasus yang BERBEDA, dari ingatan, TANPA membuka kode/modul/AI sebelumnya** — task itu ditandai dengan **[Ulangi Tanpa Contek]**. Task belum `done` kalau ronde pengulangan ini dilewati, walaupun ronde pertama sudah jalan. (Bagian `map`/`filter` di bawah — task hari Selasa 18 Agustus — tetap versi biasa, tidak ikut dirombak.)

- `map` & `filter` — **Selasa 18 Agustus** (dipecah jadi 3 sub-task):
...
- `reduce` & `find` — **Rabu 19 Agustus** (dipecah jadi 6 sub-task — **ini yang paling sering disalahpahami**, mirip kejadian `fr`/`vh` dan `this` arrow function di minggu-minggu lalu: `reduce` BUKAN cuma "for-loop yang lebih ribet ditulis", tapi cara berpikir akumulasi — bagian ini dapat pengulangan paling banyak; **mulai di sini rombak pola pengulangan wajib mulai berlaku**):
  ...
  - [ ] <!-- id: M2.W2.T2.4 --> **[Ulangi Tanpa Contek]** TANPA membuka kode `T2.1`/`T2.2` atau modul, ulangi dari nol praktik `find` + ketiga variasi `reduce` (sum, max/min, rekap object), tapi pakai **1 topik data baru** yang kamu pilih sendiri (beda dari data `map`/`filter` hari Selasa — misal array produk toko dengan harga & stok, array transaksi, dsb; boleh dipakai lagi untuk task DOM/event di hari-hari berikutnya minggu ini). Catat jujur bagian mana yang sempat lupa
  - [ ] <!-- id: M2.W2.T2.5 --> [Wajib Refleksi] Teach-back ke mentor (lisan, TANPA membaca catatan): tambahkan `console.log(accumulator, elemenSekarang)` di dalam salah satu `reduce` kamu, jalankan, lalu jelaskan ke mentor apa yang terjadi di **setiap langkah/iterasi** — buktikan kamu paham prosesnya bertahap, bukan cuma tahu hasil akhirnya
  - [ ] <!-- id: M2.W2.T2.6 --> [Wajib Refleksi] Kuis mandiri TANPA modul/AI/catatan (5 soal `reduce`/`find`, minimal 1 soal berupa potongan kode `reduce` yang harus kamu telusuri manual nilai accumulator-nya baris per baris sebelum menjawab hasil akhirnya) — baru cek jawaban setelahnya
- DOM manipulation dasar — **Kamis 20 Agustus** (dipecah jadi 6 sub-task — termasuk ronde pengulangan):
  ...
  - [ ] <!-- id: M2.W2.T3.5 --> **[Ulangi Tanpa Contek]** TANPA membuka kode `T3.2`/`T3.3` atau modul, render **data baru dari `T2.4`** (produk/transaksi, dsb — bukan data siswa) ke DOM dari nol — bikin elemen lewat `createElement`, pakai `textContent` untuk isinya. Catat jujur bagian mana yang sempat lupa command-nya
...
- Event handling — **Jumat 21 Agustus** (dipecah jadi 6 sub-task — termasuk ronde pengulangan):
  ...
  - [ ] <!-- id: M2.W2.T4.5 --> **[Ulangi Tanpa Contek]** TANPA membuka kode `T4.3` atau modul, ulangi event delegation dari nol di data dari `T3.5` — kali ini tambahkan **fitur baru** yang belum pernah dicoba sebelumnya: tombol "hapus" di tiap item yang juga ditangani lewat event delegation (bukan listener per-item). Catat jujur bagian mana yang sempat lupa/ragu
...
- [ ] <!-- id: M2.W2.T5 --> **[Uji Ulang Pemahaman Mandiri — LIVE, kasus dadakan]** Sesi terpisah SEBELUM mengerjakan proyek mandiri `T6`: mentor kasih 1 kasus kombinasi baru secara dadakan (gabungan minimal 2 dari `map`/`filter`/`reduce`/`find` + render ke DOM + event delegation) yang **belum pernah kamu kerjakan di task manapun minggu ini** — dikerjakan LANGSUNG di depan mentor (atau direkam layar kalau async), **TANPA membuka modul/AI/catatan/kode task sebelumnya**. Ini bukan untuk dinilai benar/salah sempurna, tapi untuk memverifikasi jujur pemahamannya sudah nempel atau masih hafalan/contek dari task-task sebelumnya — kalau macet total di suatu bagian, itu sinyal bagian itu perlu diulang lagi sebelum lanjut ke `T6`.
