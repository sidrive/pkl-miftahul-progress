...
>
> Minggu ini fokus utamanya **melatih kecepatan analisa bug** — dari evaluasi `M2.W4.T7` kelihatan Gazi butuh waktu cukup lama untuk menemukan penyebab error saat mengerjakan modifikasi dadakan (lupa 1 parameter di pemanggilan fungsi `render`), dan pola serupa ("butuh waktu lama cari bug", "salah ketik", "penempatan kode salah") berulang di hampir setiap minggu Bulan 2. Bagian B di bawah — bug hunting — sengaja jadi bagian terbesar minggu ini.

- Dasar debugging (dipecah jadi 4 sub-task):
  - [ ] <!-- id: M2.W5.T1.1 --> Baca & artikan minimal 5 jenis error message browser yang paling sering muncul (`TypeError: Cannot read properties of undefined`, `ReferenceError: x is not defined`, `TypeError: x is not a function`, `SyntaxError`, `Uncaught (in promise)`) — untuk tiap jenis, jelaskan apa yang errornya bilang dan kemungkinan penyebab paling umum. Lihat daftar & contoh di modul
  - [ ] <!-- id: M2.W5.T1.2 --> Praktik `debugger;` + breakpoint di DevTools (Sources tab): pasang breakpoint di 1 fungsi dari project to-do list minggu lalu, step through baris demi baris, amati nilai variabel di panel Scope — bandingkan dengan cara lama (taruh `console.log` di banyak tempat)
  - [ ] <!-- id: M2.W5.T1.3 --> Praktik strategi **bisection** buat mempersempit lokasi bug: kalau kode 50 baris error-nya gak jelas di mana, taruh 1 `console.log` di TENGAH dulu (bukan di awal), lihat apakah titik itu sempat ke-jalanin atau tidak, baru persempit ke separuh yang bermasalah — ulangi sampai ketemu barisnya persis
  - [ ] <!-- id: M2.W5.T1.4 --> **[Wajib Refleksi]** Jelaskan dengan kata sendiri: kenapa strategi bisection (mempersempit dari tengah) lebih cepat dibanding baca ulang semua kode dari baris pertama tiap kali ada bug — kaitkan ke pengalaman nyata minggu-minggu sebelumnya waktu kamu butuh waktu lama cari bug
- Bug Hunting — inti minggu ini (dipecah jadi 3 sub-task, **paling besar porsinya**):
  - [ ] <!-- id: M2.W5.T2.1 --> Buka kode **"Daftar Belanja"** di modul (sengaja berisi beberapa hal yang tidak berjalan sesuai harapan — jumlah tidak diberi tahu, cari & temukan semua sendiri). Untuk TIAP masalah yang ditemukan, catat: gejala yang terlihat, cara kamu melacak lokasinya (baca error console? bisection? breakpoint?), dan perbaikannya. **Catat juga total waktu pengerjaan dari mulai buka file sampai semua beres**
  - [ ] <!-- id: M2.W5.T2.2 --> **[Ulangi Tanpa Contek]** Buka kode **"Daftar Kontak"** di modul (kode BEDA, kategori masalah beda dari `T2.1`) — TANPA membuka `T2.1` atau modul bagian debugging lain, ulangi proses yang sama: temukan & perbaiki semua, catat gejala/cara melacak/perbaikan tiap satu, dan waktu total. **Bandingkan waktunya dengan `T2.1`** — targetnya lebih cepat, karena strategi (bukan hafalan kode) yang harusnya terbawa
  - [ ] <!-- id: M2.W5.T2.3 --> **[Wajib Refleksi]** Bandingkan `T2.1` vs `T2.2`: bug mana yang paling cepat ditemukan, mana yang paling lama, dan kenapa — pola/strategi apa dari `T2.1` yang beneran kepakai lagi di `T2.2` dan bikin lebih cepat (atau kalau tidak lebih cepat, kenapa)
- [ ] <!-- id: M2.W5.T3 --> **[Wajib Refleksi]** Kuis mandiri TANPA modul/AI/catatan — review komprehensif SELURUH Bulan 2 (10 soal campuran: `let`/`const`/arrow function, template literal, destructuring, `map`/`filter`/`reduce`/`find`, DOM manipulation, event delegation, Promise/`async`/`await`, `fetch`, `localStorage`) — baru cek jawaban setelahnya, koreksi jujur kalau ada yang salah
- [ ] <!-- id: M2.W5.T4 --> **Evaluasi Minggu ini & Penutup Bulan 2 (satu-satunya sesi bareng mentor minggu ini):** ceritakan hasil bug hunting `T2.1`/`T2.2` (waktu & pola yang ditemukan), **mentor akan sengaja menyuntik 1 bug ke to-do list yang sudah jalan (live, di depan Gazi) dan menghitung waktu sampai ketemu & terbaiki** — dibandingkan dengan waktu di `T2.1`/`T2.2` untuk lihat ada perkembangan atau belum, jawab soal live variasi dari kuis `T3`, refleksi capaian keseluruhan Bulan 2 (ES6+ sampai `localStorage`) + preview singkat Bulan 3 (Vue 3) sebagai penutup + isi refleksi capaian/kesulitan di modul
