...
- Review Bulan 2 (dipecah jadi 2 sub-task, masing-masing diselipkan 1 potongan kode kecil buat dicari bug-nya dulu sebelum lanjut ke soal kuis — bukan aplikasi penuh terpisah seperti sebelumnya):
  ...
  - [ ] <!-- id: M2.W5.T2.2 --> **[Wajib Refleksi]** Kuis review bagian 2 (DOM/event/Promise-`async`-`await`/`fetch`/`localStorage`, 5 soal) — sebelum jawab, ada 1 potongan kode kecil lain (event delegation atau `localStorage`) yang ada 1 bug tersembunyi, temukan & perbaiki dulu, baru jawab soal kuisnya. Catat waktu juga, bandingkan dengan `T2.1`
