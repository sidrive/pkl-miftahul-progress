...
- DOM manipulation dasar — **Kamis 20 Agustus** (dipecah jadi 6 sub-task — termasuk ronde pengulangan):
  - [ ] <!-- id: M2.W2.T3.1 --> Ambil elemen dari HTML — **2 putaran, data/HTML berbeda tiap putaran**: putaran 1 `document.querySelector`/`querySelectorAll`, bandingkan dengan `getElementById`/`getElementsByClassName` yang lebih lama; putaran 2 ulangi dengan struktur HTML/selector yang berbeda dari putaran 1
  - [ ] <!-- id: M2.W2.T3.2 --> Bikin & sisipkan elemen baru secara dinamis — **2 putaran, data berbeda tiap putaran**: putaran 1 pakai `document.createElement`, `appendChild`/`append`, set atribut (`setAttribute`, `classList.add`) untuk render data siswa; putaran 2 ulangi untuk render **data lain** (boleh dari `T2.1`/`T2.4`)
  - [ ] <!-- id: M2.W2.T3.3 --> Ubah isi elemen: `textContent` vs `innerHTML` — praktik keduanya untuk menampilkan data yang sama, **ulangi dengan 2 string percobaan berbeda** (salah satunya sengaja berisi tag HTML) supaya kelihatan jelas bedanya
